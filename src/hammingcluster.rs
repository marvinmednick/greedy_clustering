...
pub struct HammingClusteringInfo {
    hamming_clusters : HashMap<u32,VertexInfo>,
...
impl ClusteringInfo {
    ...
    pub fn new() -> Self { 
        ClusteringInfo { 
            hamming_clusters: HashMap::<u32,VertexInfo>::new(),
            vertex_map: HashMap::<u32,u32>::new(),
            groups: 0 
        }
    }

    // TODO -- continue to implement add... 
    pub fn add_vertex(&mut self,vertex_id: u32,hamming_code: u32)  {
        if !self.vertex_map.contains_key(&vertex_id) {
            vertex_map.insert(&vertex_id,hamming_code);
        }
    } else if ! self.vertex_map.contains_key(&vertex_id) {
